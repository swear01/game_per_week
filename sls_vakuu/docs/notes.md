...
- **Preserved Fog 必須保存牌組 snapshot**：目前起始遺物順序中 Preserved Fog 後面還有 SereTalon、DistinguishedCape，它們會新增詛咒/Apparitions。延後選牌若直接使用當時牌組，會讓後加入的牌變成可選；協調器必須在 `AfterObtained` 保存可移除卡片，之後用 `CardSelectCmd.FromDeckGeneric` 的 filter 限制候選。
