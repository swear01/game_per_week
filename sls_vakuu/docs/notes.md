...
- **目前不要對真實 profile 跑 `vakuu_test.py`**：runner 備份的是 `settings.parent/saves/progress.save`，但 v0.111.0 實際檔案在 `profile1/saves/progress.save`，而且沒有備份／還原 `current_run.save`。2026-08-23 的中斷測試已用 SHA-256 guard 的 `SteamRemoteStorage.FileDelete` 清掉測試 current run；在 runner 修正前，只跑靜態／單元測試或使用明確的拋棄式 profile。
