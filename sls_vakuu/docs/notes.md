...
- **實機 runner 要還原「原本不存在」的檔案**：只複製 `progress.save` 不足以保護 profile，因為測試會建立 `current_run.save`，Steam Cloud 也可能在遊戲退出時同步它。runner 現在備份帳號的 settings、一般／modded profile 與完整 `profile1` 目錄，cleanup 先移除測試後狀態再還原 snapshot；目錄 symlink 必須先以 `is_symlink()` 分流並保留 link target，不能交給 `rmtree`。真實 profile 仍須在測試前後用 SHA-256 guard 查驗 RemoteStorage，不能只看本機 cleanup。
